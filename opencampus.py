...
model.add(Conv2D(32, (20, 20), input_shape=(X_train.shape[1:])))
...
model.fit(X_train, y_train, epochs=5, batch_size=32, callbacks=[history])
...
for i in range(len(campus_list)):
    ...
    # if label == result[0]:
    #     ok_count += 1.
    prediction.append([campus_name[i], result[0][0]])
